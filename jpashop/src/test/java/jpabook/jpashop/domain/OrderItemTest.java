...
class OrderItemTest {
    ...
    @Test
     void 상품주문_재고수량초과() throws Exception{
         //given
        Member member = createMember();
        Book itemBook = createBook("시골 JPA", 10000, 10);

        int orderCount = 11;
        //when

         //then
        Assertions.assertThatThrownBy(() -> orderService.order(member.getId(), itemBook.getId(), orderCount)).isInstanceOf(NotEnoughStockException.class);
